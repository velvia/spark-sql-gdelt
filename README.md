...
Launch a spark-shell session, with this jar in the `SPARK_CLASSPATH`, then type the following two commands:
...
(substitute the s3n path for the path to your file location)

This loads, converts, and caches the GDELT raw data into a Spark SQL table called "gdelt".  You could then run SQL queries against it:
